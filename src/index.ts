...
const MY_DOMAIN = "wileyzhang.com";
...
const SLUG_TO_PAGE: Record<string, string> = {
	" ": "Wiley-4ccb5c8b285c4eee8e385711f51bc204",
};

const PAGE_ID: string = "Wiley-4ccb5c8b285c4eee8e385711f51bc204";

/* Step 3: enter your page title and description for SEO purposes */
const PAGE_TITLE: string | undefined = "Fruition";
const PAGE_DESCRIPTION: string | undefined =
...
export default {
	async fetch(request: Request) {
		const base = "https://wileyzhang.notion.site";
		...
		if (pathname === "/") {
			return Response.redirect("https://" + MY_DOMAIN + "/" + PAGE_ID, 301)
		} else {
			const destinationURL = `${base}${newPathName}${search}`;
			...
			const modifiedText = text.replace(/old-text/g, 'new-text')
...
async function fetchAndApply(request: Request) {
	...
	if (currentUrl.includes("4ccb5c8b285c4eee8e385711f51bc204")) {
		console.log("主页url");
		console.log(currentUrl);
		// return fetch(currentUrl.replace(MY_DOMAIN, "wileyzhang.notion.site"), request)
	}
	...
	} else if (url.pathname.includes(PAGE_ID)) {
...
async function appendJavascript(res: Response, SLUG_TO_PAGE) {
